...
\]
The difference here is the parameter $A_{ij}$, which represent the interval of resource allocation levels ($i$) corresponding to a particular query execution plan ($j$). Its intervals are obtained during the initial phase, when the query optimizer is called with different resource allocations and returns different query execution plans with their respective costs.

Both of the equations presented work within their cost model. Nevertheless, a physical  machine has more than one type of resource. Therefore the author extends the cost equations to multiple resources. He deals with the case in which $M$ resources are recommended, such that $M-1$ resources can be modelled using a linear function, while $1$ resource is modelled using a piecewise-linear function.